...
/**
 * This class is responsible of storing and managing the different entities in the game with the use
 * of a data structure that maps every {@link List} of entities, each one has a different type, to
 * their relative label which is an element of the {@link GameEntities} enumeration that states the
 * typo of the {@link List} associated. Also note that this is model side.
 */
public class EntityContainer extends AbstractContainer<GameEntity> implements EntityManager {
  ...
  public Optional<List<Enemy>> getEnemies() {
    ...
    if (!enemies.isEmpty() && enemies.get(0) instanceof Enemy) {
      return Optional.ofNullable(enemies.stream().map(e -> (Enemy) e).collect(Collectors.toList()));
    }
    return Optional.empty();
  }
  /** {@inheritDoc} */
  ...
  public Optional<List<PickupItem>> getItems() {
    ...
    if (!items.isEmpty() && items.get(0) instanceof PickupItem) {
      return Optional.ofNullable(
          items.stream().map(e -> (PickupItem) e).collect(Collectors.toList()));
    }
    return Optional.empty();
  }
  /** {@inheritDoc} */
  ...
  public Optional<List<ObstacleImpl>> getObstacles() {
    ...
    if (!obstacles.isEmpty() && obstacles.get(0) instanceof ObstacleImpl) {
      return Optional.ofNullable(
          obstacles.stream().map(e -> (ObstacleImpl) e).collect(Collectors.toList()));
    }
    return Optional.empty();
  }
  /** {@inheritDoc} */
  ...
  public Optional<List<Weapon>> getWeapons() {
    ...
    if (!weapons.isEmpty() && weapons.get(0) instanceof Weapon) {
      return Optional.ofNullable(
          weapons.stream().map(e -> (Weapon) e).collect(Collectors.toList()));
    }
    return Optional.empty();
  }
  /** {@inheritDoc} */
  ...
  public Optional<List<Bullet>> getBullets() {
    ...
    if (!bullets.isEmpty() && bullets.get(0) instanceof Bullet) {
      return Optional.ofNullable(
          bullets.stream().map(e -> (Bullet) e).collect(Collectors.toList()));
    }
    return Optional.empty();
  }
  /** {@inheritDoc} */
  ...
  public Optional<List<Platform>> getPlatforms() {
    ...
    if (!platforms.isEmpty() && platforms.get(0) instanceof Platform) {
      return Optional.ofNullable(
          platforms.stream().map(e -> (Platform) e).collect(Collectors.toList()));
    }
    return Optional.empty();
  }
  /** {@inheritDoc} */
  ...
  public final boolean deleteEntity(final ImmutablePosition2D pos) {
    for (final Entry<GameEntities, Optional<List<GameEntity>>> entry :
        this.getContainer().entrySet()) {
  ...
  public static class Builder implements EntityManagerBuilder {
    ...
    /**
     * Thanks to this constuctor, this class can be used to build pre-existent {@link
     * EntityManager}(s). The Builder pattern intent is beyond that since it's a creational pattern,
     * so if you want to create from zero a new object check the constructor up here.
     *
     * @param entityManager
     */
