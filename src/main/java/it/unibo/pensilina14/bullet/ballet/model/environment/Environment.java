...
/**
 * This interface wraps all the virtual game world and permits interaction between objects inside a
 * virtual map.
 */
public interface Environment {
   ...
   *     <p>The {@link PhysicalObject} to be deleted is first searched and then removed.
   * @return true if a {@link PhysicalObject} has been deleted. false otherwise. Could be false if
   *     there was no object at position..
   */
  boolean deleteObjByPosition(ImmutablePosition2D targetPos);

  /**
   * This must recall every {@link AbstractDynamicComponent}'s {@link
   * AbstractDynamicComponent#updateState} method, in order to update the whole environment.
   */
  void updateState();

  /**
   * Sets the event listener for the environment, so it can notify events to the listener aka the
   * controller.
   *
