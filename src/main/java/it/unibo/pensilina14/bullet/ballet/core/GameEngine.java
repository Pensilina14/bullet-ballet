...
import it.unibo.pensilina14.bullet.ballet.model.environment.events.*;
...
/**
 * Manages a variety of game aspects such as input commands processing, event handling and putting
 * into communication model and view.
 *
 * <p>This class could be considered the core of the game itself.
 */
public class GameEngine implements InputController, GameEventListener, Engine {
  ...
  /**
   * Data structure, for instance a {@link List}, whose goal is to store all the incoming events
      ...
      this.viewController =
          Optional.of(
              new ViewControllerImpl(
                  Optional.of(
                      new MapScene(this.modelController.get().getGameState().get(), this))));
  ...
  private void playerHitsObstacleEventHandler(final GameEvent e) {
    ...
    player.decreaseHealth(obstacle.getMass() / 2);
  }
  ...
  private void playerHitsEnemyEventHandler(final GameEvent e) {
    ...
    if (!enemy.isAlive()) {
      this.viewController
          .get()
          .getGameView()
          .deleteEnemySpriteImage(
              new MutablePosition2Dimpl(
                  enemy.getPosition().get().getX(), enemy.getPosition().get().getY()));
    }
  }
  ...
  private void playerHitsPickUpObjEventHandler(final Environment env, final GameEvent e)
    ...
    if (item.getItemId().equals(Items.HEART)) {
      ...
      this.modelController
          ...
          .get()
          .getCurrentScore()
          .increase(ScoreSystem.ScoreBonus.COLLECT_COIN.getBonus());
      ...
      Save.saveGameStatistics(
          this.modelController.flatMap(ModelController::getGameState).get().getPlayerName(),
          player.getCurrentScore().showScore() + player.getCurrentScore().showScore() * multiplier,
    ...
    ((PlayerHitsItemEvent) e).getItem().getEffect().applyEffect(player);
    // Update environment
        ...
        if (player.getWeapon().get().getTypeOfWeapon().equals(weapon.getTypeOfWeapon())) {
          ...
          env.deleteObjByPosition(
              new ImmutablePosition2Dimpl(
                  weapon.getPosition().get().getX(), weapon.getPosition().get().getY()));
          this.viewController
              .get()
              .getGameView()
              .deleteWeaponSpriteImage(weapon.getPosition().get());
        } else {
          player.getWeapon().get().setOff();
          player.removeWeapon();
          env.deleteObjByPosition(
              new ImmutablePosition2Dimpl(
                  actualWeapon.getPosition().get().getX(),
                  actualWeapon.getPosition().get().getY()));
          this.viewController
              .get()
              .getGameView()
              .deleteWeaponSpriteImage(actualWeapon.getPosition().get());
          weapon.setOn();
  ...
  private void bulletHitsEnemyEventHandler(final Environment env, final GameEvent e) {
    ...
    this.modelController
        ...
        .get()
        .getCurrentScore()
        .increase(ScoreSystem.ScoreBonus.KILL_ENEMY.getBonus());
  }
  ...
  private void bulletHitsObstacleEventHandler(final Environment env, final GameEvent e) {
    ...
    final MutablePosition2D bulletPos =
    ...
    this.modelController
        ...
        .get()
        .getCurrentScore()
        .increase(ScoreSystem.ScoreBonus.DESTROY_OBSTACLE.getBonus());
  }

  private void bulletHitsPlatformEventHandler(final Environment env, final GameEvent e) {
    final MutablePosition2D bulletPos =
        ((BulletHitsPlatformEvent) e).getBullet().getPosition().get();
    env.deleteObjByPosition(new ImmutablePosition2Dimpl(bulletPos.getX(), bulletPos.getY()));
    this.viewController.get().getGameView().deleteBulletSpriteImage(bulletPos);
  }
  ...
  private void gameOverEventHandler(final GameEvent e) throws IOException {
    ...
    Save.saveGameStatistics(
        this.modelController.flatMap(ModelController::getGameState).get().getPlayerName(),
        player.getCurrentScore().showScore(),
        formatter.format(date));
    this.stop();
  }
