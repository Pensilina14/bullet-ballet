...
public enum SpecialEffects implements SpecialEffectInfo {
  ...
  /**
   * SpecialEffects element specific field that represents the step in milliseconds between each
   * effect hit.
   */
  private final Steps msStep;
  /**
   * SpecialEffects element specific field that describes the duration of the whole {@link Effect}.
   */
  private final Durations msDuration;

  SpecialEffects(
      final String name, final Deltas delta, final Steps step, final Durations duration) {
    this.name = name;
    this.delta = delta;
    this.msStep = step;
