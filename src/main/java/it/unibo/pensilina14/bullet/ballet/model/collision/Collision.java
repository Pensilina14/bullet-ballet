...
public final class Collision {
   ...
   * @param whoHits
   * @param whoGetsHit
   * @return the side where the collision thing happened wrapped in an {@link Optional}. If the two
   *     objects do not collide an {@link Optional#empty()} is returned.
   */
  public static Optional<CollisionSides> getCollisionSide(
      final PhysicalObject whoHits, final PhysicalObject whoGetsHit) {
    if (whoHits.getPosition().get().getY()
        <= whoGetsHit.getPosition().get().getY()
            - (whoGetsHit.getDimension().get().getHeight() / 2)) {
      return Optional.of(CollisionSides.SOUTH);
    } else if (whoHits.getPosition().get().getY()
        >= whoGetsHit.getPosition().get().getY()
            + (whoGetsHit.getDimension().get().getHeight() / 2)) {
  ...
  public static boolean areColliding(
      final PhysicalObject firstObject, final PhysicalObject otherObject) {
    return Collision.checkAxisX(firstObject, otherObject)
        && Collision.checkAxisY(firstObject, otherObject);
  }

  private static boolean checkAxisX(
      final PhysicalObject firstObject, final PhysicalObject otherObject) {
  ...
  private static boolean checkAxisY(
      final PhysicalObject firstObject, final PhysicalObject otherObject) {
