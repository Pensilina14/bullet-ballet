...
public class GameEnvironment implements Environment {
  ...
  /**
   * This constructor uses {@link #DEFAULT_DIM} so use it wisely. Ideally we don't want this if not
   * for <strong>testing</strong> purposes.
   */
   ...
   * @param height
   * @param width
   * @param container is the {@link EntityContainer} to be set as the one and only in this game
   *     environment.
   * @param l is the event listener that is going to "listen" to the events launched in this {@link
   *     Environment}.
   */
  public GameEnvironment(
      ...
      final double height,
      final double width,
      final EntityManager container,
      final GameEventListener l) {
    this.gravity = gravity;
    this.dimension = new Dimension2Dimpl(height, width);
  ...
  private void checkGravity() {
     ...
     */
    this.entities.getEnemies().get().stream()
        .forEach(
            e -> {
              if (!e.hasLanded()) {
  ...
  private void checkCollisions() {
    ...
    eventCheckers.putAll(
        Map.of(
            "playeritem",
                new CollisionEventChecker(
                    this.entities.getItems().get(), List.of(this.entities.getPlayer().get())),
            "playerenemy",
                new CollisionEventChecker(
                    this.entities.getEnemies().get(), List.of(this.entities.getPlayer().get())),
            "playerobstacle",
                new CollisionEventChecker(
                    this.entities.getObstacles().get(), List.of(this.entities.getPlayer().get())),
            "playerplatform",
                new CollisionEventChecker(
                    this.entities.getPlatforms().get(), List.of(this.entities.getPlayer().get())),
            "enemyplatform",
                new CollisionEventChecker(
                    this.entities.getPlatforms().get(), this.entities.getEnemies().get())
            // "playerweapon", new CollisionEventChecker(this.entities.getWeapons().get(),
            // List.of(this.entities.getPlayer().get()))
            ));
    if (this.entities.getWeapons().isPresent()) {
      eventCheckers.put(
          "playerweapon",
          new CollisionEventChecker(
              this.entities.getWeapons().get(), List.of(this.entities.getPlayer().get())));
    }
    if (this.entities.getBullets().isPresent()) {
      eventCheckers.put(
          "bulletEnemy",
      ...
      eventCheckers.put(
          "bulletObstacle",
          new CollisionEventChecker(
              this.entities.getBullets().get(), this.entities.getObstacles().get()));
    }
