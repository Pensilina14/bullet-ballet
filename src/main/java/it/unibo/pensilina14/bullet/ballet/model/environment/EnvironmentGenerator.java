...
//                        break;
                    case LevelEntity.GUN:
                        this.env.get().getEntityManager().addWeapon(this.weaponFactory.createGun(new SpeedVector2DImpl(new MutablePosition2Dimpl(j * PLATFORM_SIZE, i * PLATFORM_SIZE), 1.0), this.env.get()));
                        break;
                    case LevelEntity.SHOTHUN:
                    	this.env.get().getEntityManager().addWeapon(this.weaponFactory.createShotGun(new SpeedVector2DImpl(new MutablePosition2Dimpl(j * PLATFORM_SIZE, i * PLATFORM_SIZE), 1.0), this.env.get()));
                    	break;
                    case LevelEntity.AUTOGUN:
                    	this.env.get().getEntityManager().addWeapon(this.weaponFactory.createAuto(new SpeedVector2DImpl(new MutablePosition2Dimpl(j * PLATFORM_SIZE, i * PLATFORM_SIZE), 1.0), this.env.get()));
                    	break;
                    case LevelEntity.PLAYER:
                    	//final MutablePosition2D platPos = this.env.getPlatforms().get().get(0).getPosition();
                        this.env.get().getEntityManager().setPlayer(Optional.of(this.charactersFactory.createRandomPlayer(new SpeedVector2DImpl(new MutablePosition2Dimpl(j * PLATFORM_SIZE, i * PLATFORM_SIZE), 2.0), this.env.get())));
                    	break;
